...
const UpdateBook = () => {
  ...
  const [prices, setPrices] = useState({}); // Use an object to store prices for each book
  ...
  const getRequestsData = async () => {
    try {
      if (selected === "all") {
        ...
        const response = await axios.get(
          `${process.env.REACT_APP_API_URL}/books/all/${selected}`
        );
        setReqData(response?.data);
      }
    } catch (err) {
  ...
  const handleUpdate = async (bookId) => {
    try {
      const response = await axios.patch(
        `${process.env.REACT_APP_API_URL}/books/update`,
        {
          bookId: bookId,
          price: prices[bookId], // Use prices[bookId] to get the price for the specific book
        }
      );
            ...
            <th>
              <tr>
                ...
                >
                  ...
                  <option value="Leadership & Entrepreneurs">
          ...
          {reqData?.map((book, idx) => (
            ...
            >
              ...
              <td className="p-3 text-center border-[0.5px] border-r-slate-400 w-[100%] whitespace-nowrap ">
                <input
                  type="number"
                  value={prices[book?._id] || book?.price}
                  onChange={(e) =>
                    setPrices({
                      ...prices,
              ...
              <td className="p-3 text-center font-semibold border-[0.5px] border-r-slate-400 w-[100%] whitespace-nowrap ">
                <button
                  ...
                  className="w-[100px] bg-blue-500 p-1 rounded-md shadow-md"
                >
                  Update
                </button>
              </td>
